    ...
    <div className="flex-1 flex flex-col h-full bg-claude-bg overflow-hidden select-none" id="github-workspace-root">
      ...
      {!token ? (
        ...
        <div className="flex-1 flex flex-col overflow-y-auto" style={{ background: '#0D0D0F' }}>
          ...
          {/* Search + List */}
          <div className="max-w-2xl mx-auto w-full px-4 py-4 flex flex-col gap-3" id="github-repo-list-panel">

            {/* Search bar */}
            <div className="relative">
              <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: '#6E7681' }} />
              <input
                type="text"
                ...
                style={{
                  background: '#161B22',
                  border: `1px solid ${repoSearch ? '#7C3AED' : '#30363D'}`,
                  color: '#C9D1D9',
                  boxShadow: repoSearch ? '0 0 0 3px rgba(124,58,237,0.1)' : 'none'
                }}
              />
            </div>

            {errorMsg && (
              <div className="flex items-start gap-2.5 p-3 rounded-xl text-xs" style={{ background: 'rgba(248,81,73,0.06)', border: '1px solid rgba(248,81,73,0.2)', color: '#F85149' }}>
                <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />
                <span>{errorMsg}</span>
              </div>
            )}
            ...
            <div className="flex flex-col gap-2">
              {loadingRepos ? (
                <div className="flex flex-col items-center justify-center py-16 gap-3" style={{ color: '#8B949E' }}>
                  <RefreshCw className="w-7 h-7 animate-spin" style={{ color: '#7C3AED' }} />
                  <span className="text-sm font-semibold">Loading repositories...</span>
                </div>
              ) : filteredRepos.length === 0 ? (
                <div className="text-center py-12 rounded-xl text-sm" style={{ border: '1px dashed #30363D', color: '#6E7681' }}>
                  {repoSearch ? 'No matching repositories found.' : 'No repositories on this account.'}
                </div>
              ) : (
                filteredRepos.map(repo => (
                  <button
                    ...
                    className="w-full flex items-center justify-between text-left p-4 rounded-xl transition-all cursor-pointer group"
                    style={{ background: '#161B22', border: '1px solid #21262D' }}
                    onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = '#7C3AED'; (e.currentTarget as HTMLButtonElement).style.background = '#1C1F26'; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = '#21262D'; (e.currentTarget as HTMLButtonElement).style.background = '#161B22'; }}
                  >
                    <div className="min-w-0 pr-3">
                      <div className="flex items-center gap-2 mb-1">
                        <Github className="w-3.5 h-3.5 shrink-0" style={{ color: '#8B949E' }} />
                        <span className="text-sm font-semibold text-white truncate">{repo.name}</span>
                        <span className="text-[10px] px-1.5 py-0.5 rounded-full font-mono shrink-0" style={{ background: '#21262D', color: '#8B949E', border: '1px solid #30363D' }}>
                          {repo.owner.login}
                        </span>
                      </div>
                      <p className="text-xs truncate" style={{ color: '#6E7681' }}>
                        {repo.description || 'No description provided.'}
                      </p>
                    </div>
                    <ChevronRight className="w-4 h-4 shrink-0" style={{ color: '#6E7681' }} />
                  </button>
                ))
              )}
            </div>
            ...
            {/* Footer stats */}
            <div className="flex items-center justify-between pt-1 text-[11px]" style={{ color: '#6E7681' }}>
              <span>{filteredRepos.length} of {repos.length} repositories</span>
              <button onClick={loadRepos} className="flex items-center gap-1 cursor-pointer hover:text-white transition-colors font-semibold">
